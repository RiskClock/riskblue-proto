DROP FUNCTION IF EXISTS public.get_project_list_summaries(integer, integer);
...
RETURNS TABLE(
  id uuid, name text, user_id uuid, created_at timestamp with time zone,
  status text, credits_consumed integer, report_file_path text,
  report_file_name text, workbench_status text, creator_name text,
  creator_email text, user_role project_role, tenant_id uuid
)
LANGUAGE sql
STABLE
