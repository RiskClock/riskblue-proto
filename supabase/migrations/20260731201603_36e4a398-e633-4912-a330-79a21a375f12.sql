...
BEGIN
  ...
  SELECT user_id INTO v_source_user_id FROM public.projects WHERE id = p_source_project_id;
  ...
  -- Copy project roles, skipping the owner role that the trigger already created
  ...
  WHERE project_id = p_source_project_id
    AND user_id IS DISTINCT FROM v_source_user_id;
...
REVOKE EXECUTE ON FUNCTION public.clone_project(uuid, text) FROM anon;
REVOKE EXECUTE ON FUNCTION public.clone_project(uuid, text) FROM PUBLIC;
